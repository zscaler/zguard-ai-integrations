"""
Portkey + Zscaler AI Guard — Azure OpenAI test.

Flow:
  1. Pre-scan the prompt with AI Guard SDK (direction=IN).
  2. If allowed, send through Portkey gateway to Azure OpenAI.
...
def send_via_portkey(user_content: str) -> dict:
    ...
    headers = {
        ...
        "Authorization": env["azure_api_key"],
        "x-portkey-azure-resource-name": env["azure_resource"],
        "x-portkey-azure-deployment-id": env["azure_deployment"],
        "x-portkey-azure-api-version": env["azure_api_version"],
    }
...
def main():
    ...
    print("\n=== Step 2: Portkey Gateway Request (Azure OpenAI) ===")
